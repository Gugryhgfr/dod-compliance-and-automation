...
VCFN-9X-000086_api:
  cmd.run:
    - name: 'curl -ksS -X POST ''https://localhost/api/v1/node/services/ssh/notify_mpa?action=stop'''

VCFN-9X-000086_api_2:
  cmd.run:
    - name: 'curl -ksS -X PUT ''https://localhost/api/v1/node/services/ssh'' -H ''Content-Type: application/json'' -d ''<<lookup(''template'', ''./nsx_manager_update_node_ssh_service.json.j2'')>>'''
