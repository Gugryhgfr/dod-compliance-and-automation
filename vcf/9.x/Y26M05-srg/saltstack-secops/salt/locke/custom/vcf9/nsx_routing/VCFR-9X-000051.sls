...
# Control: VCFR-9X-000051
# Title:   The VMware Cloud Foundation NSX Tier-0 Gateway must be configured to restrict it from accepting outbound IP packets
# Severity:high  CCI:CCI-001097  NIST:SC-7 a
#
...
VCFR-9X-000051_api:
  cmd.run:
    - name: 'curl -ksS -X PATCH ''https://localhost/policy/api/v1<<item.path>>'' -H ''Content-Type: application/json'' -d ''<<lookup(''template'', ''./nsx_routing_t0_configure_urpf_on_interface.json.j2'')>>'''
