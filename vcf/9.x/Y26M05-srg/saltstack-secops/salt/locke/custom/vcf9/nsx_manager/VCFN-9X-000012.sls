...
# Control: VCFN-9X-000012
# Title:   The VMware Cloud Foundation NSX Manager must be configured to enforce the limit of three consecutive invalid logon 
...
VCFN-9X-000012_api:
  cmd.run:
    - name: 'curl -ksS -X PUT ''https://localhost/api/v1/node/aaa/auth-policy'' -H ''Content-Type: application/json'' -d ''<<lookup(''template'', ''./nsx_manager_update_auth_policy_lockout.json.j2'')>>'''
