...
VCFM-9X-000066:
  file.replace:
    ...
    - pattern: '^\s+"mod_status",?$'
    - repl: ''
    - backup: False

VCFM-9X-000066_step2:
  file.replace:
    - name: /etc/lighttpd/modules.conf
    - pattern: '(^server\.modules\s+)(=|\+=\s+)(.*)("mod_status",?\s?)(.*$)'
    - repl: \1\2\3\5
    - append_if_not_found: True
    - backup: False

VCFM-9X-000066_step3:
  file.replace:
