...
VCFR-9X-000110_api:
  cmd.run:
    - name: 'curl -ksS -X PATCH ''https://localhost/policy/api/v1<<item.path>>'' -H ''Content-Type: application/json'' -d ''<<lookup(''template'', ''./nsx_routing_t0_disable_multicast_gateway.json.j2'')>>'''
