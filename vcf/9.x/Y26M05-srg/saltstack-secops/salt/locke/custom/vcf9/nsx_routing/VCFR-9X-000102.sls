...
# Control: VCFR-9X-000102
# Title:   The VMware Cloud Foundation NSX Tier-0 Gateway must be configured to advertise a hop limit of at least 32 in Router
...
VCFR-9X-000102_api:
  cmd.run:
    - name: 'curl -ksS -X PATCH ''https://localhost/policy/api/v1<<item.path>>'' -H ''Content-Type: application/json'' -d ''<<lookup(''template'', ''./nsx_routing_t0_update_ipv6_ndra_profile.json.j2'')>>'''
