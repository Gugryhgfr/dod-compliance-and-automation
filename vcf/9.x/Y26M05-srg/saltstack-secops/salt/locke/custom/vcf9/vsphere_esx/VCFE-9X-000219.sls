...
VCFE-9X-000219:
  cmd.run:
    - name: 'pwsh -NoProfile -Command "Get-VirtualSwitch | Get-SecurityPolicy | Set-SecurityPolicy -ForgedTransmits $false"'
    - unless: 'pwsh -NoProfile -Command "$v=(Get-VirtualSwitch | Get-SecurityPolicy); if($v){exit 0}else{exit 1}"'
{% else %}
VCFE-9X-000219_manual:
  test.succeed_with_changes:
    - name: 'API control VCFE-9X-000219: enable pillar vcf_secops:enable_powercli or review manually.'
{% endif %}
