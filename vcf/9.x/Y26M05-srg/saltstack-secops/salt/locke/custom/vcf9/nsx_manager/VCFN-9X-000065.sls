...
# Control: VCFN-9X-000065
# Title:   The VMware Cloud Foundation NSX Manager must record time stamps for audit records that can be mapped to Coordinated
...
VCFN-9X-000065_api:
  cmd.run:
    - name: 'curl -ksS -X PUT ''https://localhost/api/v1/node'' -H ''Content-Type: application/json'' -d ''<<lookup(''template'', ''./nsx_manager_update_node_timezone.json.j2'')>>'''
