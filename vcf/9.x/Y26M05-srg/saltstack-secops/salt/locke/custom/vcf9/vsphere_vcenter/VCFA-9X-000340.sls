...
VCFA-9X-000340:
  cmd.run:
    ...
    - unless: 'pwsh -NoProfile -Command "$v=(Get-VDSwitch | select Name,@{N=\"Port Mirroring Sessions\";E={$_.ExtensionData.Config.VspanSession.Name}}); if($v){exit 0}else{exit 1}"'
{% else %}
VCFA-9X-000340_manual:
  test.succeed_with_changes:
    - name: 'API control VCFA-9X-000340: enable pillar vcf_secops:enable_powercli or review manually.'
{% endif %}
