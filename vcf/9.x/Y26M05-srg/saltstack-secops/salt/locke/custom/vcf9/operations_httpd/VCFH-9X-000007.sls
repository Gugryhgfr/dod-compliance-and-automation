...
# Control: VCFH-9X-000007
# Title:   The VMware Cloud Foundation Operations Apache HTTP service must generate, at a minimum, log records for system star
...
VCFH-9X-000007_step4:
  file.append:
    - name: /etc/httpd/conf/httpd.conf
    - text: 'LogFormat {% raw %}"%h %l %u %t \"%r\" %>s %b" {% endraw %}common'

VCFH-9X-000007_step5:
  file.replace:
    - name: /etc/httpd/conf/httpd.conf
    - pattern: '(?!^#)(^\s*LogFormat\s*)(.*)(common)$'
    - repl: '\1{% raw %}"%h %l %u %t \"%r\" %>s %b" {% endraw %}\3'
    - append_if_not_found: True
    - backup: False

VCFH-9X-000007_step6:
  file.replace:
    - name: /etc/httpd/conf/httpd.conf
