...
VCFV-9X-000208:
  cmd.run:
    ...
    - unless: 'pwsh -NoProfile -Command "$v=(Get-VM | Where {($_.ExtensionData.Config.MigrateEncryption -eq \"disabled\")}); if($v){exit 0}else{exit 1}"'
{% else %}
VCFV-9X-000208_manual:
  test.succeed_with_changes:
    - name: 'API control VCFV-9X-000208: enable pillar vcf_secops:enable_powercli or review manually.'
{% endif %}
